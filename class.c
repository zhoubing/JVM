...
int vm_class_load_bytecode (struct vm_class *class, char *buffer, long buffer_size)
{
  ...
  class->fields = malloc (sizeof (struct vm_field) * class->field_count);
  ...
  class->methods = malloc (sizeof (struct vm_method) * class->method_count);
...
{
  struct vm_class *class = malloc (sizeof (struct vm_class));
