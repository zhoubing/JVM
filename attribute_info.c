...
AttributeBase *new_attribute(Klass *klass) {
    ...
    attributeBase->attribute_name_index = klass->bytecode_reader->read_16bit(klass->bytecode_reader);;
    attributeBase->attribute_length = klass->bytecode_reader->read_32bit(klass->bytecode_reader);
    ...
    if (strcmp(attribute_name->str, "SourceFile") == 0) {
        ...
        sourceFile->sourcefile_index = klass->bytecode_reader->read_16bit(klass->bytecode_reader);
        ...
        signature->signature_index = klass->bytecode_reader->read_16bit(klass->bytecode_reader);
        ...
        code->max_stack = klass->bytecode_reader->read_16bit(klass->bytecode_reader);
        code->max_locals = klass->bytecode_reader->read_16bit(klass->bytecode_reader);
        code->code_length = klass->bytecode_reader->read_32bit(klass->bytecode_reader);
        code->code = klass->bytecode_reader->read_bytes(klass->bytecode_reader, code->code_length);

        code->exception_table_length = ReadU2(klass);
        code->exception_table = malloc(sizeof(Exception) * code->exception_table_length);
        for (int i = 0; i < code->exception_table_length; i++) {
            memcpy(&code->exception_table[i], klass->bytecode_reader->read_bytes(klass->bytecode_reader, sizeof(Exception)), sizeof(Exception));
        }
        uint16_t attributes_count = ReadU2(klass);
        ...
        for (int i = 0; i < attributeBase->attribute_length; i++) {
            uint8_t number_of_entries = klass->bytecode_reader->read_16bit(klass->bytecode_reader);
            int numberofentries = number_of_entries;
            for (int j = 0; j < number_of_entries; j++) {
                uint8_t stack_map_frame = ReadU1(klass);
        ...
        for (int i = 0; i < attributeBase->attribute_length; i++) {
            klass->bytecode_reader->read_8bit(klass->bytecode_reader);
        }
    }
