...
#define ISTORE(index)  int val = OperandStack_PopInt(frame->operand_stack); \
                        LocalVars_SetInt(frame->localvars, index, val);

#define ILOAD(index)  int val = LocalVars_GetInt(frame->localvars, index);\
...
void pop2(Frame *frame, int *val1, int *val2) {
    *val2 = OperandStack_PopInt(frame->operand_stack);
    *val1 = OperandStack_PopInt(frame->operand_stack);
}
...
INSTRUCTION_EXEC(aconst_null) {
    OperandStack_PushRef(frame->operand_stack, 0);
    return 0;
}
...
INSTRUCTION_EXEC(iconst_0) {
    OperandStack_PushInt(frame->operand_stack, 0);
    return 0;
}
...
INSTRUCTION_EXEC(iadd) {
    ...
    OperandStack_PushInt(frame->operand_stack, val1 + val2);
    return 0;
}
...
INSTRUCTION_EXEC(iinc) {
    int val = LocalVars_GetInt(frame->localvars, frame->localvars->_operand._index.index);
    val += frame->localvars->_operand._const;
    LocalVars_SetInt(frame->localvars, frame->localvars->_operand._index.index, val);
    return 0;
}
